/*
 * mm.c - Version v80plus: explicit free list + in-place realloc optimization.
 *
 * ==============================================================
...
{
    /* New desired internal block size. */
    size_t asize;
    /* Current block size (includes overhead). */
    size_t oldsize;
    /* Temporary size holder for merge attempts. */
    size_t total;
    /* Size leftover after satisfying new request. */
    size_t remain;
    /* Extra heap extension amount when block touches epilogue. */
    size_t extendsize;
    /* Bytes to copy in fallback path. */
    size_t copySize;
    /* Pointer to physically next block. */
    char *next_bp;
    /* Pointer returned by fallback mm_malloc. */
    char *newptr;
    ...
    /* realloc(ptr, 0) == free(ptr), then return NULL. */
    if (size == 0) {
        mm_free(ptr);
        return NULL;
    }

    /* Normalize requested payload size to internal aligned block size. */
    asize = adjust_block_size(size);
    /* Read current block size for comparison and copy-length decisions. */
    oldsize = GET_SIZE(HDRP(ptr));

    /* Try case A: in-place shrink (new size <= current size). */
    if (asize <= oldsize) {
        /* Compute free tail produced by shrinking. */
        remain = oldsize - asize;
        /* Split only if tail can form legal free block. */
        if (remain >= MINBLOCKSIZE) {
            /* Commit new smaller allocated size in place. */
            PUT(HDRP(ptr), PACK(asize, 1));
            PUT(FTRP(ptr), PACK(asize, 1));

            /* Tail block starts right after resized block. */
            next_bp = NEXT_BLKP(ptr);
            /* Mark tail as free. */
            PUT(HDRP(next_bp), PACK(remain, 0));
            PUT(FTRP(next_bp), PACK(remain, 0));
            /* Coalesce the tail with following free block if present. */
            coalesce(next_bp);
        }
        /* Reuse same pointer when shrinking. */
        return ptr;
    }

    /* For grow attempts, inspect physically adjacent next block. */
    next_bp = NEXT_BLKP(ptr);

    /* Try case B: grow into next block if next block is free. */
    if (!GET_ALLOC(HDRP(next_bp))) {
        /* Combined bytes if we merge current + next block. */
        total = oldsize + GET_SIZE(HDRP(next_bp));
        /* If merged size can satisfy request, grow in place. */
        if (total >= asize) {
            /* Next block is currently in free list; unlink it before merge. */
            remove_free_block(next_bp);
            /* Temporarily mark merged region as one allocated block. */
            PUT(HDRP(ptr), PACK(total, 1));
            PUT(FTRP(ptr), PACK(total, 1));

            /* Split tail if merged block is larger than needed. */
            remain = total - asize;
            if (remain >= MINBLOCKSIZE) {
                /* Keep requested part allocated at front. */
                PUT(HDRP(ptr), PACK(asize, 1));
                PUT(FTRP(ptr), PACK(asize, 1));
                /* Build free tail block and coalesce/insert it. */
                next_bp = NEXT_BLKP(ptr);
                PUT(HDRP(next_bp), PACK(remain, 0));
                PUT(FTRP(next_bp), PACK(remain, 0));
                coalesce(next_bp);
            }
            /* Grown in place: payload prefix is preserved automatically. */
            return ptr;
        }
    }

    /*
     * Try case C: if next is epilogue, extend heap and retry in-place growth.
     * This helps avoid copy when block sits at heap end.
     */
    if (GET_SIZE(HDRP(next_bp)) == 0) {
        /* Extend by at least needed delta, but chunk-sized for amortization. */
        extendsize = MAX(asize - oldsize, CHUNKSIZE);
        if (extend_heap(extendsize / WSIZE) != NULL) {
            /* After extension, recompute physical next block. */
            next_bp = NEXT_BLKP(ptr);
            /* It should now be free; if so, same merge logic as case B. */
            if (!GET_ALLOC(HDRP(next_bp))) {
                total = oldsize + GET_SIZE(HDRP(next_bp));
                if (total >= asize) {
                    remove_free_block(next_bp);
                    ...
                    /* Extended-and-merged in place successfully. */
                    return ptr;
                }
            }
        }
    ...
    /* Case D fallback: allocate new block, copy payload bytes, free old block. */
    newptr = mm_malloc(size);
    if (newptr == NULL) {
        /* Preserve old block untouched on failure. */
        return NULL;
    }

    /* Copy only payload bytes, never metadata; clip by new requested payload size. */
    copySize = oldsize - OVERHEAD;
    if (size < copySize) {
        copySize = size;
    }
    /* Perform data move and release old block. */
    memcpy(newptr, ptr, copySize);
    mm_free(ptr);

    /* Return pointer to newly allocated resized block. */
    return newptr;
}
